...
def simulate(s,Bnum,indexing,Cor):
 ...
 results=np.zeros((len(x)+2,len(times)));hvec=[]
 x=np.append(x,[x[2],x[3]])
 ti=0
 # loop
 for t in times:
 ...
 Uig=results[4,:]; Vig=results[5,:]
 Uisvec=np.column_stack((Uis+Uig,Vis+Vig))
 PD['hvec']=hvec
...
def body(Bnum,indexing,numtaps,Cor):
  ...
  if (mod=='ExplicitEuler'):
    ...
    logging.info("Using embedded model for simulation.")
